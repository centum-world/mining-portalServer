    ...
    connection.query(query, [mining.p_userid], (err, results) => {
        if (!err) {
            ...
            if (results.length > 0) {
                ...
                bcrypt.compare(mining.p_password, results[0].p_password, function (err, result) {
...
exports.updatePartnerData = (req, res) => {
    ...
    connection.query(query, [partner.p_name, partner.p_phone, partner.p_aadhar, partner.p_email
        , partner.p_address, partner.p_state, partner.p_nominee_name, partner.p_nominee_aadhar,
    partner.p_phone, partner.p_dob, partner.p_userid], (err, results) => {
        if (!err) {
            return res.status(200).json({
...
exports.fetchPartnerWalletDetails = (req, res, next) => {
    ...
    const partnerid = req.body;
    query = "select partner_wallet,p_reffered_id,partner_status, p_liquidity,partner_count,login_counter from mining_partner where p_userid = ?";
    connection.query(query, [partnerid.p_userid], (err, results) => {
        ...
        if (!err) {
            ...
            updatequery = " update mining_partner set login_counter = ? where p_userid = ?";
            connection.query(updatequery, [login_counter, partnerid.p_userid], (err, results) => {
            ...
            if (status) {
                let selectquery = " select login_counter from mining_partner where p_userid = ? ";
                                            ...
                                            try {
                                                ...
                                                if (!err) {
                                                    ...
                                                    if (month_count < 11) {
                                                        ...
                                                        connection.query(query, [partner_wallet, wallet_update_date, partner_count, perday_partner_wallet_amount, partnerid.p_userid], (err, results) => {

                                                            try {
                                                                if (!err) {
                                                                    // console.log("13")
                                                                    let insertquery = "insert into partner_wallet_history (walletAmount,wallet_update_date,p_userid) values (?,?,?)";
                                                                    connection.query(insertquery, [walletAmount, new Date(), partnerid.p_userid], (err, results) => {


                                                                        console.log('289', partnerid, new Date());
                                                                        try {
                                                                            if (!err) {
                                        ...
                                        if (table_flag === 'create_member') {
                                            ...
                                            connection.query(selectquery, [reffered_id], (err, results) => {
                                                ...
                                                if (!err) {
                                                    ...
                                                    let activePartnerCountQuery = "select count(*) AS activePartner from mining_partner join create_member on mining_partner.p_reffered_id = create_member.reffer_id WHERE  mining_partner.p_reffered_id = ? AND mining_partner.partner_status ='1'";
                                                    connection.query(activePartnerCountQuery, [reffered_id], (err, results) => {
                                                        //try {
                                                        if (!err) {
                                                            activePartnerCount = Number(results[0]?.activePartner);
                                                    ...
                                                    connection.query(insertquery, [walletAmount, wallet_update_date, m_userid], (err, results) => {
                                                        ...
                                                        if (!err) {
                                                            ...
                                                            connection.query(updatequery, [member_wallet, wallet_update_date, member_count, perday_member_wallet_amount, added_wallet, m_userid], (err, results) => {

                                                                if (!err) {
                                                                    if (activePartnerCount === 0) {
                                                                        let setTotalWalletZero = "update create_member set member_count =?, member_wallet =?,perday_member_wallet_amount=?,added_wallet = ? where m_userid =?";
                                                                        connection.query(setTotalWalletZero, [member_count = 0, member_wallet = 0, perday_member_wallet_amount = 0, added_wallet = 0, m_userid], (err, results) => {
...
exports.fetchPartnerApproveWithdrawalHistoryForPartner = (req, res) => {
    ...
    connection.query(query, [partnerId.p_userid], (err, results) => {
        if (!err) {
            return res.status(200).json({
                message: "Fetched Partner Withdrawal History for Particular Partner successfully",
                data: results
            });
        } else {
            return res.status(500).json(err);
...
exports.partnerForgetPassword = (req, res) => {
    ...
    connection.query(query, [partnerId.p_userid], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            connection.query(updatequery, [otp, partnerId.p_userid], (err, results) => {

                if (!err) {
                    forgetpasswordSms(phone, { "type": 'Partner', "userid": partnerId.p_userid, "otp": otp })
...
exports.partnerRegeneratePassword = (req, res) => {
    ...
    connection.query(selectquery, [partnerId.p_userid], (err, result) => {
        ...
        bcrypt.hash(partnerId.p_password, 10, function (err, result) {
            ...
            updatequery = "update mining_partner set p_password = ? where p_userid = ?"
            connection.query(updatequery, [hash, partnerId.p_userid], (err, results) => {

                if (!err) {



                    sms(phone, { "type": 'Partner', "userid": partnerId.p_userid, "password": password })
