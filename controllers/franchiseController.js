  ...
  try {
    ...
    connection.query(
      ...
      (error, result) => {
        ...
        let fetchVerifyDate =
          "select verifyDate from create_franchise where franchiseId = ? ";
        connection.query(fetchVerifyDate, [franchiseId], (err, result) => {
          if (result.length > 0) {
        ...
        cron.schedule("* * * * *", () => {
          ...
          if (currentTimestamp - lastExecutionTimestamp >= 30 * 24 * 3600 * 1000) {
            ...
            console.log("Running logic after 30 days!");
